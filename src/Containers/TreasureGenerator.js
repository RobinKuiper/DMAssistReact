...
export default class TreasureGenerator extends Component {
  ...
  calculateGold(){
    var ratings = [this.state.challenge_rating]
    let treasure = {
      cp: 0,
      sp: 0,
      ep: 0,
    ...
    let goldToTrade = ((this.state.percentage*1)/100)*total

    if(this.state.percentage > 0 && goldToTrade >= 10){
  ...
  render = () => (
    <main>
      <Panel title='Treasure Generator' content={this.content.bind(this)} />
    </main>
  )
  ...
  content() {
    ...
    return (
      <div>
        <Grid relaxed='very'>
          <Grid.Column width={3}>
            <Form onValidSubmit={this.calculateGold.bind(this)}>
              <Form.Select
            ...
            <Button style={{marginTop: 10}} type='submit' positive content='Generate Treasure' icon='diamond' onClick={this.calculateGold.bind(this)} />
          </Grid.Column>
