...
export default class Monsters extends Component {
  ...
  renderMobileContent = () => {
    ...
    return (
      <div>
        <Input style={{marginBottom: 30}} fluid icon='search' placeholder='Search name, alignment, size, type, etc.' value={this.state.searchQuery} onChange={this.search.bind(this)} />
              ...
              loader={Loader}>
                <Item.Group divided>
                  { monsters.sort(this.compare.bind(this)).slice(0, this.state.limit*(this.state.page+1)).map(monster => (
                        <MonsterItem monster={monster} encounters={this.encounters} encounter={this.state.encounter} />
                  ))}
                </Item.Group>
          </InfiniteScroll>
          ) : (
          ...
          <Table.Body>
            { monsters.length > 0 ?
                monsters.sort(this.compare.bind(this)).slice(this.state.page*this.state.limit, this.state.limit*(this.state.page+1)).map(monster => (
                  <MonsterTableItem monster={monster} encounters={this.encounters} encounter={this.state.encounter} />
                ))
              : (
                <Table.Row>
