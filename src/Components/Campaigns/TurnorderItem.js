...
import { Button, Input, Item, Popup } from 'semantic-ui-react'
import MonsterModal from './../Monsters/MonsterModal'
import { calculateMod } from './../../Lib/Common'
import Dice from './../../Lib/Dice'
//import AddModal from './AddModal'
import { Database } from './../../Lib/firebase'
import TurnorderItemModal from './TurnorderItemModal'
...
export default class TurnorderItem extends Component {
  ...
  handleUpdate = (e) => {
    ...
    if(e.initiative) update.initiative = e.initiative
    if(e.hit_points) update.hit_points = e.hit_points
    if(e.armor_class) update.armor_class = e.armor_class
    if(e.level) update.level = e.level

    if(e.hit_points) this.setState({ tempHP: update.hit_points })
    Database.ref('/campaigns/'+this.props.campaign.key).child('turnorder/'+this.props.item.id).update(update)
  }
      ...
      <Item key={item.key}>
        <Item.Content>
          <Item.Header className={item.done && 'strikethrough'}>
            <TurnorderItemModal handleUpdate={this.handleUpdate} item={item} trigger={<span style={{textDecoration: 'underline', cursor: 'pointer'}}>{item.name}</span>} />
          </Item.Header>
