...
export const MonsterTableItem = ({ monster, encounters, encounter }) => (
    <Table.Row key={monster.key}>
      ...
      { Auth.currentUser && encounter &&
        <Table.Cell>{(<Popup content='Add to encounter' trigger={<Button icon='plus' size='mini' onClick={() => encounters.addMonster(monster) } />} />)}</Table.Cell>
      }
    </Table.Row>
)